...
void EfficiencyFX(Int_t nGen = 1E5) {
  gRandom->SetSeed();
  TH1F *hGen = new TH1F("hGen", "Generated Events", 100, -1, 6);
  TH1F *hAcc = new TH1F("hAcc", "Accepted Entries", 100, -1, 6);
  hGen->Sumw2();
  hGen->SetFillColor(kAquamarine);
  hAcc->Sumw2();
  hAcc->SetFillColor(kOcean);
  TF1 *eff = new TF1("eff", "0.2 * x", -1, 6);
